...
(function () {
  ...
  function refreshBar() {
    ...
    ui.status.textContent = undoStack.length ? undoStack.length + (undoStack.length === 1 ? ' change' : ' changes') : '';
  }
  ...
  function init() {
    ...
    ui.bar = el('div', { class: 'bar', hidden: true, role: 'toolbar', 'aria-label': 'Page editor' }, [
      ui.msg, ui.status, ui.undo, ui.redo,
    ...
    [ui.hl, ui.sel, ui.panel, ui.bar, ui.toggle].forEach(function (n) { root.appendChild(n); });
    root.appendChild(el('style', { text: '[hidden]{display:none!important}' }));
    document.body.appendChild(host);
    document.addEventListener('keydown', onKey, true);

    window.PageEditor = { start: start, stop: stop, undo: undo, redo: redo,
